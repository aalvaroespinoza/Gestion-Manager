...
export default function InventoryPage() {
  ...
  const handleAdjustStock = (productId: string, newStock: number, reason: string) => {
    ...
    showToast(`Stock actualizado correctamente (${reason}).`, "info")
  }
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      ...
      if (searchQuery.trim()) {
        ...
        const matchExtra = Object.values(product.extraAttributes || {}).some(
          (val) => String(val).toLowerCase().includes(q)
        )
        if (!matchSku && !matchName && !matchDesc && !matchExtra) return false
      }
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Gestión integral de catálogo, medidas, talles y existencias por rubro comercial.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="hover:border-blue-300 dark:hover:border-blue-800 transition-colors">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-slate-900 dark:text-slate-100">{products.length} productos</div>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{totalStockCount} unidades en bodegas</p>
          </CardContent>
        </Card>
        ...
        <Card className="hover:border-blue-300 dark:hover:border-blue-800 transition-colors">
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Bajo el umbral mínimo</p>
          </CardContent>
        </Card>
        ...
        <Card className="hover:border-blue-300 dark:hover:border-blue-800 transition-colors">
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Requiere reposición urgente</p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardHeader className="space-y-4 pb-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
            <div className="sm:col-span-5">
              <Input
                placeholder="Buscar por nombre, SKU o atributo técnico..."
            ...
            <div className="sm:col-span-2">
              <Select
                ...
                options={[
                  ...
                  { label: "Stock Crítico", value: "LOW_STOCK" },
          ...
          <Table className="border-0 rounded-none">
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Producto & Rubro</TableHead>
                <TableHead>Atributos Dinámicos (Medidas / Talles)</TableHead>
                ...
                paginatedProducts.map((product) => {
                  ...
                  return (
                    <TableRow key={product.id}>
                      ...
                      {/* Dynamic Extra Attributes Summary */}
                      <TableCell className="max-w-xs">
                        <div className="flex flex-wrap gap-1.5">
                          {Object.entries(product.extraAttributes || {}).map(([key, val]) => {
                            ...
                            return (
                              <span
                                key={key}
                                className="inline-flex items-center text-[10px] bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2 py-0.5 rounded-md text-slate-700 dark:text-slate-300"
                              >
