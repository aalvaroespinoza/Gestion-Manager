...
export default function VentasPage() {
  ...
  const [selectedClient, setSelectedClient] = useState<ClientSelectOption>(mockClients[0])
  ...
  // Sales History State
  const [salesHistory, setSalesHistory] = useState<InvoiceData[]>([
    {
      id: "sale-hist-1",
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Terminal rápida de mostrador para atención y cobro con tickets digitales.
          </p>
        </div>
                  ...
                  {cart.length === 0 ? (
                    <div className="py-10 text-center text-slate-400 space-y-2">
                      ...
                      <p className="text-[11px] text-slate-400">
                        Haz clic en los productos del catálogo izquierdo para agregarlos.
                      </p>
                    </div>
                  ) : (
                    cart.map((item) => {
